...
public class Cryptography {
	...
	public class func getExistingKey(id: String) throws -> CryptographicKey? {
		...
		if error == errSecSuccess || error == errSecDuplicateItem || error == errSecInteractionNotAllowed {
			if let res = result as? SecKey {
				return CryptographicKey(key: res, keyId: id)
			}
			return nil
		}
	...
	public class func generateKey(id: String, bits: UInt16 = 256, storeInKeychain: Bool = true, secureEnclave: Bool = true, controlFlags: SecAccessControl? = Cryptography.accessControlFlags) throws -> CryptographicKey? {
